...
class OrderRepository(BaseRepository, IOrderRepository):
    ...
    async def get(self, pk: ObjectId) -> base_entity_class:
        ...
        async for item in instance.order_payments.aiterator():
            item_entity = await self.to_entity(OrderPaymentEntity, item)
            entity.payments.append(item_entity)
        return entity
    ...
    async def set_payment_status(self, pk: ObjectId, status: PaymentStatusChoices) -> None:
        instance = await self.get_queryset().aget(pk=pk)
        instance.payment_status = status
        await instance.asave(update_fields=["payment_status", "updated_at"])
